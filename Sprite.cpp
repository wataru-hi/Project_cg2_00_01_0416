...
void Sprite::draw(DirectXCommon* dxCommon,D3D12_GPU_DESCRIPTOR_HANDLE textureSrvHandle)
{
	...
	dxCommon_->GetCommandList()->IASetVertexBuffers(0, 1, &vertexBufferVier);
	dxCommon_->GetCommandList()->IASetIndexBuffer(&indexBufferVier);

	dxCommon_->GetCommandList()->SetGraphicsRootConstantBufferView(0, materialResource->GetGPUVirtualAddress());
	dxCommon_->GetCommandList()->SetGraphicsRootDescriptorTable(2, textureSrvHandle);
