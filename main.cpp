	...
	ID3D12GraphicsCommandList* commandList)
{
	...
	uint64_t intermediastateSize = GetRequiredIntermediateSize(texture, 0, UINT(subresources.size()));
	ID3D12Resource* intermediastateResource = CreateBufferResource(device, intermediastateSize);
	UpdateSubresources(commandList, texture, intermediastateResource, 0, 0, UINT(subresources.size()), subresources.data());
	...
	return intermediastateResource;
}
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int)
{
	...
	const int intVertexCount = 6;

	//頂点数
	ID3D12Resource* vertexResource = CreateBufferResource(device, sizeof(VertexData) * intVertexCount);
	...
	*materialDate = Vector4(1.0f, 1.0f, 1.0f, 1.0f);
	...
	vertexBufferView.SizeInBytes = sizeof(VertexData) * intVertexCount;
	//1頂点当たりのサイズ
